...
const getASTNodeInfo = require("jsdoc/src/astnode").getInfo;
...
function collectClassInfo(extendCall, classDoclet) {
	...
	if (classInfoNode?.type === Syntax.ObjectExpression && jsdocClassName && addLeadingCommentNode) {
		...
		if (classInfoComment == null) {
			...
			debug(`lends comment injected for class '${jsdocClassName}'`);
...
exports.handlers = {
	...
	*/
	...
	newDoclet: function(e) {
		...
		if (e.doclet.experimental
			&& !e.doclet.deprecated
			&& !isSemVer(e.doclet.experimental)) {
			future(`As of version 1.139, the experimental tag is deprecated and should no longer be used. Use the @ui5-experimental-since tag instead (${e.doclet.meta?.lineno})`);
		}
	...
	visitNode: function(node, e, parser, currentSourceName) {
		...
		function processPotentialEnum(literal, comment) {
			const values = literal.properties.reduce((map, prop) => {
				// use the JSDoc API to determine a name from the property's key
				// this is necessary to get the same enumID as in the later step where
				// name info from a JSDoc doclet is used to build the enumID
				const key = getASTNodeInfo(prop).name;
				map[key] = convertValue(prop.value);
				return map;
			}, Object.create(null));
