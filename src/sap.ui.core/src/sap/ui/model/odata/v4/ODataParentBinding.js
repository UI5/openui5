...
sap.ui.define([
	...
	ODataParentBinding.prototype.fetchIfChildCanUseCache = function (oContext, sChildPath,
		...
		if (bDependsOnOperation && !sResolvedChildPath.includes("/$Parameter/")
			...
			// timing.
			// With data aggregation, no auto-$expand/$select is needed for a context referencing
			// aggregated data, or a dynamic property, but the child may still use the parent's
			// cache; in case of a single record auto-$expand/$select is used.
