...
//					}
	...
	// split would get wrong. It is already bundled and loaded (the loader itself uses URI), and
	// with the default preventInvalidHostname it does not throw on string input. hostname()
	// returns the raw-case host and keeps a trailing dot, so the normalization is applied here.
	...
	//
	// A bare entry "example.com" matches the host "example.com" itself and any
	// subdomain "*.example.com".
	//
	// A leading-dot entry ".example.com" restricts the match to its subdomains:
	// "sub.example.com" matches while "example.com" does not. This preserves the
	// legacy whitelist semantics.
	FrameOptions._matchHost = function(sHostname, sEntry) {
		...
		if (sEntry.charAt(0) === ".") {
			...
			return sHostname.slice(-sEntry.length) === sEntry;
		}
		// bare form: exact host or proper subdomain
		var sDotEntry = "." + sEntry;
		return sHostname === sEntry || sHostname.slice(-sDotEntry.length) === sDotEntry;
	};
	...
	FrameOptions.prototype._check = function(bParentResponsePending) {
		...
		// available in all supported browsers, so fall back to protocol + host.
