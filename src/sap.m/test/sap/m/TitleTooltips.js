sap.ui.require([
	...
	const oTooltipModeProvider = new MemoryConfigurationProvider();
	oTooltipModeProvider.external = true;
	BaseConfig.registerProvider(oTooltipModeProvider);

	function setEnhancedTooltipMode(bEnhanced) {
		oTooltipModeProvider.set("sapUiXxTooltip", bEnhanced ? "enhanced" : "native");
	...
	function buildSampleContent() {
		...
		const oQuickViewButton = new Button({
	...
	const oModeSwitch = new Switch({
		state: TooltipEnablement.isEnhancedTooltipEnabled(),
		customTextOn: "On",
		customTextOff: "Off",
		tooltip: "Toggle enhanced tooltips vs. native browser tooltips",
		change: function(oEvent) {
			const bEnhanced = oEvent.getParameter("state");
			setEnhancedTooltipMode(bEnhanced);
			refreshContent();
			MessageToast.show(bEnhanced
				? "Enhanced tooltips enabled."
				: "Native browser tooltips (title attribute) enabled.");
		}
	});

	const oModeToolbar = new Toolbar({
		content: [
			new Label({ text: "Enhanced tooltips:", labelFor: oModeSwitch }).addStyleClass("sapUiTinyMarginEnd"),
			oModeSwitch,
			new ToolbarSpacer(),
			new Text({ text: "Switch and compare against native browser tooltips." })
		]
	}).addStyleClass("sapUiSmallMarginBottom");
	...
	function refreshContent() {
		const aContent = oPage.getContent();
		for (let i = aContent.length - 1; i >= 2; i--) {
			oPage.removeContent(i).destroy();
		}
		buildSampleContent().forEach(function(oControl) {
