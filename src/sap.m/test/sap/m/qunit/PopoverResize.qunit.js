...
sap.ui.define([
	...
	QUnit.test("Resize Handler", async function (assert) {
		const popover = await openPopover();
		const resizeHandle = getResizeHandle(popover);

		assert.ok(resizeHandle, "resize handler is rendered");
	});
	...
	QUnit.test("PlacementType Top, OffsetX = -100, Opener Align = Start", async function (assert) {
		...
		assert.ok(popover.getDomRef().classList.contains("sapMPopoverResizeHandleTopRight"), "sapMPopoverResizeHandleBottomRight class is added");
