...
sap.ui.define([
	...
	QUnit.test("setDescription() property", function (assert) {
		...
		this.oMessagePopover.attachLongtextLoaded(function () {
			assert.strictEqual(this.oMessagePopover._oMessageView._detailsPage.getContent()[3].getContent().indexOf("h2") >= 1, true, "There should be an h2 tag");
			assert.strictEqual(this.oMessagePopover._oMessageView._detailsPage.getContent()[3].getContent().indexOf("<script"), -1, "There should be no script tag in the html");
			assert.strictEqual(this.oMessagePopover._oMessageView._detailsPage.getContent()[3].getContent().indexOf("embed"), -1, "There should be no embed tag in the html");
			doneLongtextLoaded();
