...
sap.ui.define([
	...
	function(
		...
		QUnit.module("sap.ui.mdc.chart.ChartToolbar: createSelectionDetailsBtn", {
			beforeEach: async function () {
		...
		QUnit.test("Selection Details button triggers 'beforeOpen' event", function (assert) {
			const done = assert.async();

			this.oMDCChart.setShowSelectionDetails(true);
			this.oMDCChart.initialized().then(function () {
				const oSelectionDetailsBtn = this.oMDCChart._oSelectionDetailsBtn;
				const spyBeforeOpen = sinon.spy(oSelectionDetailsBtn, "fireBeforeOpen");

				oSelectionDetailsBtn.fireBeforeOpen();
				assert.ok(spyBeforeOpen.calledOnce, "'beforeOpen' event is triggered");

				spyBeforeOpen.restore();
				done();
			}.bind(this));
		});

		QUnit.test("Selection Details button updates actions on 'beforeOpen'", function (assert) {
			const done = assert.async();
			this.oMDCChart.setShowSelectionDetails(true);
			this.oMDCChart.initialized().then(function () {
				const oItem = new Item(),
					...
					actionGroups: [],
					detailsItemActions: [],
					getSelectionDetailsActions: sinon.stub().returns({
						getDetailsItemActions: sinon.stub().returns([
							{ clone: sinon.stub().returns(oItem)}
						]),
						getDetailsActions: sinon.stub().returns([
							{ clone: sinon.stub().returns(oItem)}
						]),
				...
				const oSelectionDetailsBtn = ToolbarControlFactory.createSelectionDetailsBtn("idChart", mParams);
				const spyBeforeOpen = sinon.spy(oSelectionDetailsBtn, "fireBeforeOpen");
				sinon.stub(oSelectionDetailsBtn, "getItems").returns([
					{ clone: sinon.stub().returns(oItem), addAction: sinon.stub()}
				]);
				sinon.stub(oSelectionDetailsBtn, "addAction");
				sinon.stub(oSelectionDetailsBtn, "removeAllActions");
				sinon.stub(oSelectionDetailsBtn, "addActionGroup");
				sinon.stub(oSelectionDetailsBtn, "removeAllActionGroups");
				const oEvent = { getSource: sinon.stub().returns(oSelectionDetailsBtn) };
				oSelectionDetailsBtn.fireBeforeOpen(oEvent);

				// Assertions
				assert.ok(spyBeforeOpen.calledOnce, "'beforeOpen' event is triggered");
				assert.ok(oSelectionDetailsBtn.removeAllActions.calledOnce, "All list actions are removed");
				assert.ok(oSelectionDetailsBtn.addAction.calledWith(oItem), "List actions are updated");
				assert.ok(oSelectionDetailsBtn.removeAllActionGroups.calledOnce, "All action groups are removed");
				assert.ok(oSelectionDetailsBtn.addActionGroup.calledWith(oItem), "Action groups are updated");
