...
sap.ui.define([
	...
	QUnit.test("getModelFilter: Empty for navigation properties (Any)", function(assert) {
		...
		const oFilter = oOperator.getModelFilter(oCondition, "navPath*/propertyPath", oType, false, BaseType.String);
		const aFilters = oFilter?.getFilters();
		...
		assert.equal(aFilters?.[0]?.getOperator(), FilterOperator.All, "Filter0 operator");
		assert.ok(aFilters?.[0]?.getCondition(), "Filter0 condition");
		...
		let oConditionFilter = aFilters?.[0]?.getCondition();
		let aConditionFilters = oConditionFilter?.getFilters();
		assert.ok(oConditionFilter.isAnd(), "Condition-Filters AND combined");
		assert.equal(aConditionFilters?.length, 2, "2 Condition-Filters included");
		assert.equal(aConditionFilters?.[0]?.getPath(), "L1/propertyPath", "Condition-Filter0 path");
		assert.equal(aConditionFilters?.[0]?.getOperator(), FilterOperator.EQ, "Condition-Filter0 operator");
		assert.equal(aConditionFilters?.[0]?.getValue1(), null, "Condition-Filter0 value1");
		assert.equal(aConditionFilters?.[0]?.getValue2(), undefined, "Condition-Filter0 value2");
		assert.equal(aConditionFilters?.[1]?.getPath(), "L1/propertyPath", "Condition-Filter1 path");
		assert.equal(aConditionFilters?.[1]?.getOperator(), FilterOperator.NE, "Condition-Filter1 operator");
		assert.equal(aConditionFilters?.[1]?.getValue1(), null, "Condition-Filter1 value1");
		assert.equal(aConditionFilters?.[1]?.getValue2(), undefined, "Condition-Filter1 value2");
		oConditionFilter = aFilters?.[1]?.getCondition();
		aConditionFilters = oConditionFilter?.getFilters();
		assert.notOk(oConditionFilter.isAnd(), "Condition-Filters OR combined");
		assert.equal(aConditionFilters?.length, 2, "2 Condition-Filters included");
		assert.equal(aConditionFilters?.[0]?.getPath(), "L1/propertyPath", "Condition-Filter0 path");
		assert.equal(aConditionFilters?.[0]?.getOperator(), FilterOperator.EQ, "Condition-Filter0 operator");
		assert.equal(aConditionFilters?.[0]?.getValue1(), "", "Condition-Filter0 value1");
		assert.equal(aConditionFilters?.[0]?.getValue2(), undefined, "Condition-Filter0 value2");
		assert.equal(aConditionFilters?.[1]?.getPath(), "L1/propertyPath", "Condition-Filter1 path");
		assert.equal(aConditionFilters?.[1]?.getOperator(), FilterOperator.EQ, "Condition-Filter1 operator");
		assert.equal(aConditionFilters?.[1]?.getValue1(), null, "Condition-Filter1 value1");
		assert.equal(aConditionFilters?.[1]?.getValue2(), undefined, "Condition-Filter1 value2");
